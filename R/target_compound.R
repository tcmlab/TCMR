...
#' @param freq.value Minimum number of occurrences of herbs
#' @param digits.value number of digits after decimal point
#' @param ... additional parameters
#'
#' @return list:
#' 1. Frequency of occurrence of herbs
#' 2. Selected traditional Chinese medicine prescription results
#' @export
#'
...
#' newdata <- target_compound(COVID19_gene)
#' head(newdata[[1]])
#' head(newdata[[2]])
#' }
target_compound <- function(disease.gene,
  ...
  tcmsp2 <- tcmsp[tcmsp$Target %in% disease.gene, ] %>%
    tidyr::drop_na() %>%
